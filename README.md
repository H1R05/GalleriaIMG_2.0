...
Nota: per la ricerca metadati lato API i campi chiave sono `nomeImmagine` e `tipo`, inoltre nella root del progetto e' presente una cartella chiamata "/dati_json_mongodb/" con all'interno i dati json completi da importare su MongoDB Compass (utenti e metadati) per avere l'applicazione funzionante al 100%.
